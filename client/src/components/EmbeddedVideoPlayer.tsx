      ...
      >
        ...
        <div className="relative w-full bg-black rounded-lg overflow-hidden">
          <div className="aspect-video">
            {embedUrl ? (
              <iframe
                src={withAutoplay(embedUrl, autoplay)}
                title={video.title}
        ...
        <div className="mt-4 text-white">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-3">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center gap-1.5 rounded-md border border-white/20 bg-white/10 px-3 py-1.5 text-xs font-medium text-white hover:bg-white/20"
            >
              <ExternalLink className="h-3.5 w-3.5" />
              Open original {video.provider === "youtube" ? "YouTube" : video.provider === "facebook" ? "Facebook" : "source"}
            </a>
            <p className="text-gray-500 text-xs break-all">{video.url}</p>
          </div>
        </div>
 ...
 */
export function getEmbedUrl(video: VideoSource): string | null {
 ...
 */
export function getVideoThumbnail(video: VideoSource): string {
  const videoId = extractVideoId(video.url, video.provider);
...
/**
 * Add autoplay without corrupting provider-specific query strings.
 */
