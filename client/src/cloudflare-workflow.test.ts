...
describe("Cloudflare Pages deployment workflow", () => {
  it("deploys the intended Pages project and GitHub artifact output", () => {
    ...
    expect(workflow).toContain("cp -a dist/public/. dist/");
    expect(workflow).not.toContain("find dist/public -maxdepth 1");
  });

  it("runs reproducible checks before publishing", () => {
  ...
  it("keeps protected routes and authentication in Supabase client code", () => {
    const hostnameRouterSource = readFileSync(
      fileURLToPath(new URL("./components/HostnameRouter.tsx", import.meta.url)),
      "utf8"
    );
    expect(hostnameRouterSource).toContain('path={"/auth"}');
    expect(hostnameRouterSource).toContain('path={"/auth/callback"}');
    expect(hostnameRouterSource).toContain('path={"/dossier"}');
    expect(hostnameRouterSource).toContain('path={"/admin"}');
  ...
  it("keeps protected-page hooks stable across auth loading transitions", () => {
    ...
    expect(adminSource).toContain("return <AuthorizedAdminDashboard role={role ?? \"admin\"} />;");
  });
});
