...
function LoadingScreen() {
  return <div className="flex min-h-screen items-center justify-center bg-[#f3f5f6] text-sm text-[#66747c]">Verifying authorization…</div>;
}

function AccessDenied({ message = "This account is not authorized to view this area." }: { message?: string }) {
  const [, setLocation] = useLocation();
  return <div className="flex min-h-screen items-center justify-center bg-[#f3f5f6] px-6"><div className="max-w-md border border-[#d4dde1] bg-[#fffdfa] p-8 text-center shadow-[0_12px_32px_rgba(30,44,52,0.06)]"><h1 className="font-serif text-3xl font-semibold text-[#21313a]">Access denied</h1><p className="mt-3 text-sm leading-6 text-[#66747c]">{message}</p><button className="mt-6 bg-[#21313a] px-4 py-2.5 text-sm font-semibold text-white" onClick={() => setLocation("/")}>Return to portal</button></div></div>;
}
...
function ProtectedCaseRoute() {
  ...
  if (!user) return <Auth />;
  if (!can("can_view_dossier")) return <AccessDenied message="Your account does not have permission to view the case dossier." />;
  return <CaseReview />;
}
...
function ProtectedAdminRoute() {
  ...
  if (authLoading || authorizationLoading) return <LoadingScreen />;
  if (!user) return <Auth />;
  if (!isAdmin && !isOwner) return <AccessDenied message="Administrative access is restricted to Owner and Admin accounts." />;
